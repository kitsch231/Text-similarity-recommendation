...
def train(config, model, train_iter, dev_iter, test_iter,writer):
    ...
    optimizer_grouped_parameters = [{'params': [p for n, p in model.named_parameters() if 'bert' in n],'lr': config.bert_learning_rate},#包含bert层学习率},#包含resnet层学习率
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if config.usesloss:
                ...
                sloss=SupConLoss().to(config.device)
...
#tensorboard --logdir=bert_model/log/S_minirbt-h256 --port=6006